...
describe Koala::Utils do
  ...
  describe '.is_hashtag?' do
    context 'when hashtag' do
      it 'returns true' do
        expect(Koala::Utils.is_hashtag?('#facebook')).to eq(true)
      end
    end

    context 'when not hashtag' do
      it 'returns false' do
        expect(Koala::Utils.is_hashtag?('facebook')).to eq(false)
      end
    end
  end
